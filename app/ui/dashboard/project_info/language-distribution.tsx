...
export default function LanguageDistribution({ data }: LanguageDistributionProps) {
    if (!data || data.length === 0) {
        return (
            <Card>
                <CardContent>
                    <div className="flex justify-center items-center h-40">No language data available</div>
                </CardContent>
            </Card>
        )
    }
            ...
            <CardContent className="pt-6">
                <div className="w-full h-[250px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={formattedData} layout="vertical" margin={{ top: 5, right: 50, left: 70, bottom: 5 }}>
                            <XAxis type="number" tickFormatter={(value) => `${value}%`} domain={[0, 100]} tickCount={6} />
