...
export default function CommitFrequency({
  ...
  const { mergedCommits, hasMerged, isLoading, handleMerge, resetMerge } =
    useAuthorConsolidation(data, repoId);

  // Add state for processed data
  const [total, setTotal] = useState(0);
  const [dayEntries, setDayEntries] = useState<any[]>([]);
  const [emailToDisplayName, setEmailToDisplayName] = useState<
  ...
  const authorEntries = emailToDisplayName
    ? Object.entries(emailToDisplayName)
    : [];
        ...
        <Card className="w-full">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                onClick={resetMerge}
                title="Return to author consolidation"
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {authorEntries
                  ...
                  .map(([email, displayName], index) => {
                    ...
                    const percentage =
                      total > 0
                        ? ((authorCommits / total) * 100).toFixed(1)
                        : "0";
                    ...
                    selectedAuthors.includes("TOTAL@commits")
                      ...
                      : selectedAuthors.length > 1
                        ? selectedAuthors
                            ...
                            .join(", ")
                        : emailToDisplayName[selectedAuthors[0]] ||
                          selectedAuthors[0]
