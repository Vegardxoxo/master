...
export default function CommitQuality({ data }: { data: LLMResponse[] }) {
  ...
  useEffect(() => {
    ...
    const excellentPercentage = ((categoryCounts["Excellent"] || 0) / totalCommits) * 100
    const goodPercentage = ((categoryCounts["Good"] || 0) / totalCommits) * 100
    const needsImprovementPercentage = ((categoryCounts["Needs Improvement"] || 0) / totalCommits) * 100

    // Calculate overall quality score (0-10)
    const qualityScore = (excellentPercentage * 1 + goodPercentage * 0.6 + needsImprovementPercentage * 0.2) / 10
    ...
    const metrics = {
      ...
      qualityStatus: qualityScore >= 7 ? "good" : qualityScore >= 4 ? "moderate" : "poor",
    }
  ...
  const messageQuality = Object.entries(categoryCounts).map(([category, count]) => ({
    name: category,
    value: count,
  }))
  ...
  const filteredData = selectedCategory ? data.filter((commit) => commit.category === selectedCategory) : []
  ...
  return (
    <Card className="w-full gap-4">
      <CardHeader>
        <CardTitle className="text-xl font-bold">Commit Message Quality</CardTitle>
      </CardHeader>
      <CardContent>
        <p className="text-center text-lg font-semibold mb-4">Total Commits: {data.length}</p>
        ...
        <div className="flex flex-wrap justify-center gap-8 mt-6">
          {messageQuality.map((entry) => (
            <div key={entry.name} className="flex items-center justify-center gap-2">
              <div className="w-6 h-6 rounded-full" style={{ backgroundColor: colorMap[entry.name] }} />
              <span className="text-sm lg:text-base font-semibold">
                {entry.name}: {entry.value} ({((entry.value / data.length) * 100).toFixed(1)}%)
              </span>
            </div>
          ))}
        </div>
          ...
          <DialogContent className="sm:max-w-4xl h-auto max-h-[80vh] overflow-auto w-full">
            <DialogHeader>
              ...
              <DialogDescription className="text-lg font-semibold">
                Detailed information for commits categorized as: {selectedCategory}.
              </DialogDescription>
            </DialogHeader>
        ...
        <div className={"mt-4"}>
          <BestPractices title={"Write descriptive commit messages"} icon={"commit"} variant={"success"}>
            <ul className="space-y-1 list-disc pl-5">
              <li>
                Write descriptive commit messages that start with a verb in present tense, imperative mood (e.g., "Add
                feature" not "Added feature")
              </li>
              <li>Each commit should have a single, clear purpose explained in the message</li>
              <li>Write as if giving instructions to the codebase (e.g., "Make xyzzy do frotz")</li>
              <li>Ensure your explanation can be understood without external resources</li>
              <li>Summarize relevant points instead of just linking to external discussions</li>
              <li>Clear commit messages force teams to understand the value each change brings</li>
              <li>If you can't describe the value of a commit, reconsider if the change is necessary</li>
            </ul>
          </BestPractices>
        </div>
      </CardContent>
