...
export default function PullRequestVsIssues({
  prs,
  issues,
}: PullRequestVsIssuesProps) {
  ...
  const timelineData = useMemo(() => {
    ...
    prs.forEach((pr) => {
      ...
      const date =
        pr.created_at?.split("T")[0] || new Date().toISOString().split("T")[0];
      allDates.add(date);

      const existingData = dataByDate.get(date) || {
        pullRequests: 0,
        issues: 0,
      };
      dataByDate.set(date, {
  ...
  const metrics = useMemo(() => {
    ...
    const prToIssueRatio =
      totalIssues > 0 ? (totalPRs / totalIssues).toFixed(2) : "0";
        ...
        <CardContent className="pt-6">
          <div className="h-[250px] flex items-center justify-center">
            <p className="text-muted-foreground">
              No pull request or issue data available
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <CardContent>
        ...
        <div className="flex flex-col sm:flex-row justify-between mt-4 gap-2">
          ...
          <div className={"flex justify-end"}>
            <Badge
              variant="outline"
              className={`flex items-center gap-1 ${getRatioIndicator()}`}
            >
        ...
        <div className={"mt-4"}>
          <BestPractices
            title="Balance Planning and Implementation"
            icon="merge"
            variant="info"
          >
            <ul className="space-y-1 list-disc pl-5">
              <li>
                A healthy PR:Issue ratio (0.8-1.2) indicates good balance
                between planning and implementation
              </li>
              <li>
                Lower ratios may suggest incomplete work or abandoned issues
              </li>
              <li>
                Higher ratios could indicate work being done without proper
                planning
              </li>
            </ul>
          </BestPractices>
