...
const SECTION_ORDER: (keyof VisibleSections)[] = ["overview", "commits", "branches", "pipelines", "pullRequests"]
...
export function DashboardNavigation({ onToggle, onToggleSubsection, visibleSections }: SidebarProps) {
  ...
  const hasSubsections = (item: any): boolean => {
    ...
    return Object.keys(item).some((key) => key !== "visible" && key !== "text" && typeof item[key] === "object")
  }

  // Render subsections for a section
  const renderSubsections = (section: keyof VisibleSections, sectionData: any) => {
      ...
      <div className="mt-1 ml-7 pl-2 border-l border-gray-200">
        <ul className="space-y-1 py-1">
          {Object.entries(sectionData).map(([key, value]) => {
            ...
            if (key === "visible" || key === "text" || typeof value !== "object") return null
  ...
  return (
    <Card className="w-full bg-white shadow-md">
      <CardHeader className="pb-4 border-b">
        <CardTitle className="text-xl font-semibold text-gray-800">Dashboard Controls</CardTitle>
      </CardHeader>
          ...
          {SECTION_ORDER.map((sectionKey) => {
            ...
            return (
              ...
              >
                ...
                {canExpand && isExpanded && renderSubsections(sectionKey, value)}
              </li>
            )
          })}
        </ul>
