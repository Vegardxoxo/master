...
export default function AuthorMerger({ data, onMerge }: AuthorMergerProps) {
  ...
  const [consolidatedCommits, setConsolidatedCommits] = useState<
    CommitData[] | null
  >(null);
  ...
  const applyMappings = () => {
    ...
    const updatedCommits = commitData.map((commit) => {
      ...
      const key = `${originalName}|${originalEmail}`;
      const mappedAuthor = authorMappings[key] || {
        name: originalName,
        email: originalEmail,
      };
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      <p className="text-muted-foreground mb-4">
        Consolidate different author identities before proceeding to the
        visualization. Authors mapped to the same identity will share the same
        color when multiple authors are mapped to that identity.
      </p>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateAuthorMapping(
                            ...
                            author.email,
                            e.target.value,
                            authorMappings[`${author.name}|${author.email}`]
                              ?.email || author.email,
                          )
                        }
                      />
