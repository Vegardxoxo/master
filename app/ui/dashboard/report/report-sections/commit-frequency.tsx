...
export default function CommitFrequency({ metrics, include }: ReportSectionProps) {
  ...
  const authorStats = metrics.authorTotals
    ? Object.entries(metrics.authorTotals)
        ...
        .map(([email, commits]) => ({
          ...
          commits,
          percentage: metrics.total > 0 ? (((commits as number) / metrics.total) * 100).toFixed(1) : "0",
        }))
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Commit Frequency Visualization</CardTitle>
        </CardHeader>
        <CardContent className="pt-2">
          {metrics && metrics.url ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-muted-foreground">No commit frequency chart available.</p>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="text-sm text-muted-foreground mb-2">
                Total commits: <span className="font-medium text-foreground">{metrics.total}</span>
              </div>

              <div className="grid grid-cols-1 gap-3">
                {authorStats.map((author) => (
                  <div key={author.email} className="flex items-center justify-between border-b pb-2">
                    <div>
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-sm text-muted-foreground">{author.percentage}% of total</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {authorStats.length > 1 && (
                <div className="mt-4 p-3 bg-muted/30 rounded-md text-sm">
                  <div className="font-medium mb-1">Contribution Distribution</div>
                    ...
                    {authorStats.map((author, index) => {
                      ...
                      return (
                        <div key={author.email} className="flex items-center">
                          <div className={`w-3 h-3 rounded-full mr-1 ${colors[index % colors.length]}`} />
