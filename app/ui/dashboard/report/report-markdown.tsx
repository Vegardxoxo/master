...
`;
    ...
    if (
      commitQuality.justifications &&
      commitQuality.justifications.length > 0
    ) {
...
`;
    ...
    if (
      commitContributions.contributors &&
      commitContributions.contributors.length > 0
    ) {
...
`;
    ...
    if (
      developmentBranches.branchConnections &&
      developmentBranches.branchConnections.length > 0
    ) {
...
`;
    ...
    if (fileCoverage && fileCoverage.length > 0) {
      ...
      const lowCoverageFiles = fileCoverage.filter((file: any) => {
        ...
        return (
          statements < lowCoverageThreshold ||
          branches < lowCoverageThreshold ||
          functions < lowCoverageThreshold
        );
      });
