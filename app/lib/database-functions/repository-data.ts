...
export async function storeLanguageDistribution(
  owner: string,
  repo: string,
  repoId: string,
) {
  try {
    ...
    const operations = formattedData.map(
      (lang: { name: string; value: number; percentage: number }) =>
...
export async function storeContributors(
  ...
  try {
    ...
    const operation = contributors.map(
...
export async function getRepoInfo(owner: string, repo: string) {
  try {
    ...
    const { stars, openIssues, forks, watchers, repoName, updatedAt, url } =
      result.repository;
...
export async function storePullRequests(
  owner: string,
  repo: string,
  repoId: string,
) {
...
export async function getPullRequests(owner: string, repo: string) {
  try {
    ...
    if (prRecord && prRecord.data) {
      ...
      return {
        success: false,
        error: "No pull requests found in database for this repository.",
      };
    }
  } catch (e) {
...
export async function storeCommitsToMain(
  owner: string,
  repo: string,
  repoId: string,
) {
...
export async function storeBranches(
  owner: string,
  repo: string,
  repoId: string,
) {
...
export async function getWorkflowRuns(owner: string, repo: string) {
  const { success, repository, error } = await findRepositoryByOwnerRepo(
    owner,
    repo,
  );
...
export async function storeRepositoryMetaData(
  owner: string,
  repo: string,
  repoId: string,
) {
